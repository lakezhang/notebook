...
def decode_line(line, need_label = True) :
    ...
    features = {COLUMN_NAMES[0] : items[0:-1]}
    label = items[-1]
...
def main(_) :
    ...
    feature_columns = [tf.feature_column.numeric_column(key=col, shape=(4,)) for col in COLUMN_NAMES[:-1]]
...
if __name__ == '__main__':
    flags = tf.app.flags
    flags.DEFINE_string('model_dir', './model/iris2', 'Directory where all models are saved')
